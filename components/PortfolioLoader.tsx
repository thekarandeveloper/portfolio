...
export function PortfolioLoader() {
  ...
  useEffect(() => {
    ...
    const typing = window.setInterval(() => {
      ...
      if (index >= name.length) window.clearInterval(typing);
    }, 90);

    const zoom = window.setTimeout(() => setPhase("zoom"), 1250);
    const fade = window.setTimeout(() => setPhase("fade"), 1900);
    const gone = window.setTimeout(() => setPhase("gone"), 2600);
  ...
  return (
    <div
      className={`fixed inset-0 z-[9999] overflow-hidden bg-figma-dark transition-opacity duration-700 ${
    ...
    >
      ...
      <div className={`absolute bottom-0 left-0 top-11 z-10 w-[220px] border-r border-white/5 bg-figma-panel p-4 transition-opacity duration-500 ${phase !== "draw" ? "opacity-0" : "opacity-100"}`}>
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        <div
          className={`relative flex items-center justify-center overflow-hidden border-2 border-pink bg-bg transition-all duration-700 ${
            phase === "draw" ? "h-[260px] w-[520px] rounded-sm" : "h-screen w-screen border-0"
          }`}
        >
